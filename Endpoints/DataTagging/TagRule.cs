...
{
    public class TagRule : BaseRestApi
    {
        const string Url = "/tagging/api/TagRule";
        ...
        public override List<TestStep> GetParameterTests(MessageData messageData, string authValue, IEnvironment environment)
        {
            ...
            DataTable dataTable = DataBaseExecuter.ExecuteCommand("SQL", SecretsManager.SQLConnectionString(), query.QueryTagRules());
            foreach (DataRow dataRow in dataTable.Rows)
            {
                TestParams.TestStepName = GetType().Name + "_GET_TagRule";
                TestParams.ExpectedResponseBody = dataRow["TagRules"].ToString();
